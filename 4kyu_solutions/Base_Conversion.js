...
function convert(input, source, target) {
    // Convert input into a DECIMAL value from source
    
    const sourceLength = source.length;
    const inputLength = input.length;
    let inputValue = 0;
      for(let i=0; i<inputLength; i++){
        inputValue += (Math.pow(sourceLength, i) * source.indexOf(input[inputLength-1-i]));
      }
      
    // Find the last power value of the answer (length of answer)
    
    const targetLength = target.length;
    let lastPow = 0;
      while(inputValue>=Math.pow(targetLength, lastPow)){
        lastPow++;
      }
      if(lastPow!==0) lastPow-=1;
      
    // Find Numeric value of each index and add
    // corresponding target number or letter to ans
    
      let ans = '';
      for(let i=lastPow; i>=0; i--){
        let targetIndex = parseInt(inputValue / Math.pow(targetLength,i));
        inputValue = inputValue % Math.pow(targetLength,i);
        ans+= target[targetIndex];
      }
      return ans;
}
